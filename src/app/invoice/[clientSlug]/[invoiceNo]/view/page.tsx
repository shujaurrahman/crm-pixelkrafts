  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Case 2: Client Name Slug with ID suffix (e.g., salman-0002)
        else {
          const match = rawId.match(/-([a-zA-Z0-9]+)$/);
          if (match && match[1]) {
            // Assume the suffix is the lead ID number
            targetId = `ENQ-${match[1]}`;
          } else {
            // Fallback for older links without suffix
